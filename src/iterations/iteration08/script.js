...
// ── Payline constants ──────────────────────────────────────────────────────
// REEL_ROWS: number of visible symbol rows per reel (3-row display).
// ACTIVE_PAYLINES: all three horizontal rows are evaluated as paylines.
// Each payline is wagered at Math.floor(bet / ACTIVE_PAYLINES) tokens.
const REEL_ROWS = 3;
const ACTIVE_PAYLINES = 3;

// ── RTP & Probability constants ──────────────────────────────────────────
// Theoretical RTP: 96.50% per payline — calculated from the weighted symbol
// probabilities and the PAYOUT_TABLE multipliers. Multi-payline RTP is now
// consistent with single-line RTP because lineBet uses fractional division
// (bet / 3) and only the final per-line payout is floored. This eliminates
// the systematic rounding loss present in iteration 07.
...
// Maps each STRIP position to its SYMBOLS array index.
// Used by getRowSymbolIndices to find the symbol one row above or below the
// chosen center position without traversing the STRIP array on every spin.
 ...
 * TOTAL_WEIGHT.
 *
 * Individual symbol probabilities (TOTAL_WEIGHT = 57):
 *   DIAMOND: 1/57 = 1.75%    MONEY: 2/57 = 3.51%    AGI: 3/57 = 5.26%
 *   BRAIN:   5/57 = 8.77%    ORACLE: 4/57 = 7.02%   STORAGE: 6/57 = 10.53%
 *   POWER:   6/57 = 10.53%   TOKEN: 6/57 = 10.53%   METRICS: 6/57 = 10.53%
 *   DNA:     5/57 = 8.77%    ROCKET: 5/57 = 8.77%   TARGET: 5/57 = 8.77%
 *   WILD:    1/57 = 1.75%    SCATTER: 1/57 = 1.75%   MULTIPLIER: 1/57 = 1.75%
 *
...
/**
 * Creates a single reel cell DOM element for the given symbol using DOM
 * methods instead of innerHTML to avoid XSS risk.
 * @param {{sym: string, lbl: string}} symbol - Symbol object from SYMBOLS or STRIP
 * @returns {HTMLElement} The populated .reel-cell div
 */
 ...
 * methods instead of innerHTML to avoid XSS risk. Prepends one phantom
 * cell (mirroring STRIP's last symbol) and appends one (mirroring the
 * first symbol) so the 3-row display wraps correctly at the strip edges
 * without showing empty space in the top or bottom row.
...
function buildReel(id) {
  ...
  el.innerHTML = '';
  // Phantom top cell — mirrors STRIP's last symbol for wrap-around at top row
  el.appendChild(makeReelCell(STRIP[STRIP.length - 1]));
  STRIP.forEach(function (s) { el.appendChild(makeReelCell(s)); });
  // Phantom bottom cell — mirrors STRIP's first symbol for wrap-around at bottom row
  el.appendChild(makeReelCell(STRIP[0]));
}

/**
 * Initializes all three reels to their starting state and clears all
 * payline highlights and sidebar markers from the previous spin.
 * @returns {void}
 */
function initReels() {
...
function animateReel(reelEl, targetStripPos, duration, delay) {
  return new Promise(function (resolve) {
    setTimeout(function () {
      const totalCells = STRIP.length + 2; // +2 for the phantom top and bottom cells
...
/**
 * Given the STRIP stop position for each reel's center row, returns the
 * symbol index (into SYMBOLS) for all three visible rows of every reel.
 * Wraps around the STRIP using modulo arithmetic so edge positions (0 and
 * STRIP.length − 1) always yield a valid adjacent symbol.
 *
 * @param {number[]} stripPositions - STRIP center index for each of the 3 reels
 * @returns {number[][]} rowsByReel[reelIdx][rowIdx]
 *   rowIdx 0 = top row, 1 = center row, 2 = bottom row
 */
function getRowSymbolIndices(stripPositions) {
  const len = STRIP.length;
  return stripPositions.map(function (sp) {
    return [
      STRIP_SYMBOL_IDX[(sp - 1 + len) % len], // top row (wraps around)
      STRIP_SYMBOL_IDX[sp],                     // center row (primary payline)
      STRIP_SYMBOL_IDX[(sp + 1) % len],         // bottom row (wraps around)
    ];
  });
}

/**
 * Highlights winning payline rows inside each reel-wrap and updates the
 * numbered payline markers on both sidebars. Clears any previous highlights
 * before applying new ones.
 *
 * @param {number[]} winningLines - Indices (0–2) of paylines that produced a win
 * @param {{type: string}[]} paylineResultsArg - Full result object for each payline
 * @returns {void}
 */
function setPaylineHighlights(winningLines, paylineResultsArg) {
  ...
  // Apply colour class to each winning payline's row in every reel
  ...
  // Update numbered sidebar markers (left and right)
  [0, 1, 2].forEach(function (lineIdx) {
...
function spin() {
  ...
  return Promise.all(
    ...
    // ── Payline evaluation ────────────────────────────────────────────────
    // Build the 3×3 grid: rowsByReel[reel][row] = symbol index.
    // The center (row 1) is the symbol chosen by weightedSymbol(); top and
    // bottom are the adjacent STRIP positions, wrapping at both ends.
    const rowsByReel = getRowSymbolIndices(stripPositions);

    // paylineSymIdx[lineIdx] = [reel0sym, reel1sym, reel2sym] for that row.
    ...
    // Each payline is wagered at lineBet tokens. Uses fractional division
    // so no tokens are lost to rounding — only the final per-line payout
    // is floored. This preserves theoretical RTP across all bet amounts.
    const lineBet = bet / ACTIVE_PAYLINES;
    const paylineResults = paylineSymIdx.map(function (idxs) {
      return calcPayout(idxs);
    });

    // Aggregate across all paylines
    let totalWin = 0;
    let bestType = 'lose';
    let bestResult = paylineResults[1]; // default: center payline
    ...
    paylineResults.forEach(function (res, lineIdx) {
      ...
      // Free spins awarded at most once per spin (first qualifying payline wins)
    ...
    // Progressive jackpot grows on full losses (all paylines lose).
    // totalBurned only tracks full-loss spins.
    if (type === 'lose') {
    ...
    // ── Reel highlight — winning reels and special symbols ────────────────
    // Winner-reel glow applied when the primary (center) payline wins.
    [0, 1, 2].forEach(function (i) {
      const wrap = document.getElementById(`reel-wrap-${i}`);
      if (winningLines.indexOf(1) !== -1) wrap.classList.add('winner-reel');
      // Special symbol highlights use the center row (primary display row)
    ...
    let msg;
    if (type === 'lose') {
      msg = `${base} &minus;${bet.toLocaleString()} tokens.`;
    } else if (type === 'pair') {
      msg = `${base} +${winAmt.toLocaleString()} tokens!${lineLabel}`;
    } else {
      msg = `<span class="combo-badge">${combo}</span>${base} +${winAmt.toLocaleString()} tokens!${lineLabel}`;
    }

    // ── Near-miss check ───────────────────────────────────────────────────
    // Evaluated on the center payline (primary display row). Only fires on a
    // full loss — see checkNearMiss JSDoc for criteria.
    ...
    if (nearMiss) {
      ...
      [0, 1, 2].forEach(function (i) {
        const idx = paylineSymIdx[1][i]; // center row
        const wrap = document.getElementById(`reel-wrap-${i}`);
    ...
    // History uses center row symbols for the symbol display column
    ...
    // ── Free spins from scatter ───────────────────────────────────────────
    // Award at most SCATTER_FREE_SPINS per spin even if multiple paylines hit.
    if (freeSpinsTotal > 0) {
