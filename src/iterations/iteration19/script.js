...
function anyBlockingModalOpen() {
  const ids = ['bonus-modal', 'funds-modal', 'paytable-modal', 'autoplay-modal', 'history-modal', 'stats-modal', 'settings-modal', 'rewards-modal', 'progression-modal', 'themes-modal'];
...
function loadRewards() {
  try {
    ...
    if (d.dailyMissionsDate === today) {
      if (Array.isArray(d.dailyProgress) && d.dailyProgress.length >= 3) REWARDS.dailyProgress = d.dailyProgress.slice(0, 3);
      if (Array.isArray(d.dailyClaimed) && d.dailyClaimed.length >= 3) REWARDS.dailyClaimed = d.dailyClaimed.slice(0, 3);
    }
...
function updateRewardProgress(data) {
  ...
  if (REWARDS.dailyMissionsDate !== today) {
    ...
    REWARDS.dailyProgress = [0, 0, 0];
    REWARDS.dailyClaimed = [false, false, false];
  }
