...
function spin() {
  ...
  return Promise.all(
    ...
    const paylineSymIdx = [0, 1, 2].map(function (row) {
      return [rowsByReel[0][row], rowsByReel[1][row], rowsByReel[2][row]];
    });

    // Each payline is wagered at lineBet tokens. The full bet is still
    // deducted from balance — lineBet only determines payout scaling.
    // Exact RTP re-balancing for the rounding loss is scoped to iteration 8.
    const lineBet = Math.max(1, Math.floor(bet / ACTIVE_PAYLINES));
    ...
    paylineResults.forEach(function (res, lineIdx) {
      const lineWin = res.mult > 0 ? Math.floor(lineBet * res.mult) : 0;
      if (lineWin > 0) {
        totalWin += lineWin;
