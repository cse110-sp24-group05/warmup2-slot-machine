...
var STRIP = (function () {
  var s = [];
  for (var k = 0; k < 3; k++)
    SYMBOLS.forEach(function (sym) { s.push(sym); });
  return s;
})();
...
/**
 * Animates a single reel spinning and stopping at a target index.
 * @param {HTMLElement} reelEl - The reel-inner DOM element
 * @param {number} targetIndex - Symbol index to land on
 * @param {number} duration - Approximate spin duration in ms
 * @param {number} delay - Delay before spin starts in ms
 * @returns {Promise<void>} Resolves when animation completes
 */
function animateReel(reelEl, targetIndex, duration, delay) {
...
function spin() {
  ...
  return Promise.all(
    [0, 1, 2].map(function (i) {
      return animateReel(document.getElementById('reel' + i), indices[i], durations[i], delays[i]);
    })
  ).then(function () {
    ...
    // Progressive jackpot grows on losses
    if (type === 'lose') {
      updateProgressiveJackpot(Math.ceil(bet * 0.08));
      totalBurned += bet;
    } else {
      totalBurned += Math.max(0, bet - winAmt);
    }
