...
const SYMBOL_WEIGHTS = [1, 2, 3, 5, 4, 6, 6, 6, 6, 5, 5, 5];
...
function weightedSymbol() {
  const total = SYMBOL_WEIGHTS.reduce(function (a, b) { return a + b; }, 0);
...
let progressiveJackpot = INITIAL_JACKPOT;
let freeSpin = false;
...
function saveProgressiveJackpot() {
  try {
    localStorage.setItem(JACKPOT_STORAGE_KEY, String(progressiveJackpot));
  } catch { /* localStorage unavailable */ }
}
...
function spinBonusWheel() {
  ...
  const targetIdx = Math.floor(Math.random() * WHEEL_PRIZES.length);
  const n = WHEEL_PRIZES.length;
  const sliceAngle = (Math.PI * 2) / n;
  const fullSpins = (Math.floor(Math.random() * 4) + 5) * Math.PI * 2;
...
function reclampBet() {
  ...
  const target = balance > 0 ? Math.max(1, Math.min(old, balance)) : 1;
...
function addHistory(indices, bet, winAmt, type, combo) {
  ...
  list.innerHTML = spinHistory.slice(0, HISTORY_DISPLAY_LIMIT).map(function (h) {
    const cls = clsMap[h.type] || 'l';
    let delta;
...
function spin() {
  ...
  return Promise.all(
    ...
    try {
      const soundFn = { jackpot: soundJackpot, bigwin: soundBigWin, win: soundWin, pair: soundPair, lose: soundLose };
    ...
    if (spinCount % BONUS_WHEEL_INTERVAL === 0) {
...
progressiveJackpot = loadProgressiveJackpot();
document.getElementById('pj-amount').textContent = progressiveJackpot.toLocaleString();
