...
function weightedSymbol() {
  let factor = (typeof getDifficultyFactor === 'function') ? getDifficultyFactor() : 1;
  let adjustedWeights = SYMBOL_WEIGHTS.map(function (w, i) {
    return SYMBOLS[i].rare ? Math.max(1, Math.round(w * factor)) : w;
  });
  let total = adjustedWeights.reduce(function (a, b) { return a + b; }, 0);
...
function updateUI() {
  ...
  if (rtpEl) {
    if (STATE.totalWagered > 0) {
      ...
      rtpEl.style.color = sessionRTP >= TARGET_RTP * 100 - 5 ? 'let(--win)' : 'let(--lose)';
    } else {
      rtpEl.textContent = '\u2014';
      rtpEl.style.color = 'let(--muted)';
    }
  }
}
...
function addHistory(indices, bet, winAmt, type, combo) {
  let syms = indices.map(function (i) { return SYMBOLS[i].sym; }).join(' ');
...
function celebrate(type, winAmt, bet) {
  ...
  if (type === 'jackpot') {
    ...
    if (pj > INITIAL_JACKPOT) {
      ...
      updateUI();
      setTimeout(function () {
        setMsg(document.getElementById('msg').innerHTML + ' <span style="color:let(--jackpot);font-weight:700">+' + pj.toLocaleString() + ' JACKPOT POOL!</span>', 'jackpot');
      }, 300);
    }
...
function openStatsModal() {
  ...
  if (STATE.spinCount > 0) {
    let avgBet = Math.round(STATE.totalBetSum / STATE.spinCount);
  ...
  let net = STATE.totalWon - STATE.totalBurned;
  let netEl = document.getElementById('stat-net-profit');
  if (net > 0) {
    netEl.textContent = '+' + net.toLocaleString();
    netEl.style.color = 'let(--win)';
  } else if (net < 0) {
    netEl.textContent = '\u2212' + Math.abs(net).toLocaleString();
    netEl.style.color = 'let(--lose)';
  } else {
    netEl.textContent = '0';
    netEl.style.color = 'let(--muted)';
  }

  let bestStreak = document.getElementById('stat-best-streak');
  let curStreak = document.getElementById('stat-cur-streak');
...
function openSettingsModal() {
  let sfxCb = document.getElementById('setting-sfx');
  let voiceCb = document.getElementById('setting-voice');
  let kbCb = document.getElementById('setting-keyboard');
  ...
  let nameInput = document.getElementById('setting-player-name');
...
let saveNameBtn = document.getElementById('save-player-name-btn');
if (saveNameBtn) {
  saveNameBtn.addEventListener('click', function () {
    let inp = document.getElementById('setting-player-name');
    if (!inp) return;
...
// ── Rewards System ────────────────────────────────────────────────────────
let REWARDS_STORAGE_KEY = 'safariRewardsV1';

let DAILY_LOGIN_BASE = 75;
let WEEKLY_REWARD_BASE = 250;
let FRIEND_REWARD_BASE = 200;
let MAX_FRIEND_REFERRALS = 3;

let DAILY_MISSIONS_DEF = [
...
function rewardsWeekStr() {
  ...
  let yearStart = new Date(Date.UTC(y, 0, 1));
...
function loadRewards() {
  try {
    let stored = localStorage.getItem(REWARDS_STORAGE_KEY);
    if (!stored) return;
    let d = JSON.parse(stored);
    let today = rewardsDateStr();
    let week = rewardsWeekStr();
    ...
    if (d.dailyMissionsDate === today) {
      if (Array.isArray(d.dailyProgress)) {
        let dp = d.dailyProgress.slice(0, DAILY_MISSIONS_DEF.length);
        while (dp.length < DAILY_MISSIONS_DEF.length) dp.push(0);
        REWARDS.dailyProgress = dp;
      }
      if (Array.isArray(d.dailyClaimed)) {
        let dc = d.dailyClaimed.slice(0, DAILY_MISSIONS_DEF.length);
...
function initLoginStreak() {
  ...
  let last = new Date(REWARDS.lastLoginDate + 'T00:00:00');
  let now = new Date(today + 'T00:00:00');
  let diffDays = Math.round((now - last) / 86400000);
...
function updateRewardsBadge() {
  let btn = document.getElementById('rewards-btn');
  if (!btn) return;
  let hasPending = hasUnclaimedRewards();
  let badge = btn.querySelector('.rewards-badge');
  if (hasPending) {
    if (!badge) {
...
function renderRewardsModal() {
  ...
  let loginReward = Math.floor(DAILY_LOGIN_BASE * mult);
  ...
  let weeklyReward = Math.floor(WEEKLY_REWARD_BASE * mult);
  document.getElementById('weekly-reward-amount').textContent = '+' + weeklyReward + ' chips';
  let weeklyBtn = document.getElementById('claim-weekly-btn');
  ...
  let claimInviteBtn = document.getElementById('claim-invite-btn');
  if (REWARDS.referralsClaimed >= MAX_FRIEND_REFERRALS) {
    ...
    let invReward = Math.floor(FRIEND_REWARD_BASE * mult);
...
function renderMissionList(containerId, missions, progress, claimed, type, mult) {
  ...
  missions.forEach(function (mission, idx) {
    let prog = progress[idx] || 0;
    let isClaimed = claimed[idx];
    let isComplete = prog >= mission.target;
    let reward = Math.floor(mission.reward * mult);
    let pct = Math.min(100, Math.round((prog / mission.target) * 100));
    ...
    let rewardLbl = document.createElement('div');
...
function claimDailyLogin() {
  ...
  let mult = getRewardMultiplier();
  let reward = Math.floor(DAILY_LOGIN_BASE * mult);
...
function claimDailyMission(idx) {
  ...
  let mult = getRewardMultiplier();
  let reward = Math.floor(DAILY_MISSIONS_DEF[idx].reward * mult);
...
function claimWeekendMission(idx) {
  ...
  let mult = getRewardMultiplier();
  let reward = Math.floor(WEEKEND_MISSIONS_DEF[idx].reward * mult);
...
function claimWeeklyReward() {
  ...
  let mult = getRewardMultiplier();
  let reward = Math.floor(WEEKLY_REWARD_BASE * mult);
...
function claimFriendBonus() {
  ...
  let mult = getRewardMultiplier();
  let reward = Math.floor(FRIEND_REWARD_BASE * mult);
...
function copyInviteLink() {
  copyToClipboard(buildInviteLink(), '📋 Invite link copied! Share it with a friend.');
  playSound('click');
}

function showRewardToast(text) {
  let existing = document.querySelector('.reward-toast');
...
// ── Progression System ────────────────────────────────────────────────────
let PROGRESSION_STORAGE_KEY = 'safariProgressionV1';
let MAX_LEVEL = 60;
...
function getCurrentRank() {
  let rank = TIER_RANKS[0];
  rank.index = 0;
  for (let i = TIER_RANKS.length - 1; i >= 0; i--) {
...
function loadProgression() {
  try {
    let stored = localStorage.getItem(PROGRESSION_STORAGE_KEY);
    if (!stored) return;
    let d = JSON.parse(stored);
...
function onLevelUp(newLevel) {
  let reward = getLevelReward(newLevel);
  STATE.balance += reward;
  updateUI();
  updateSpinAvailability();
  ...
  let rank = getCurrentRank();
  let checkpoint = isCheckpointLevel(newLevel);
...
function updateProgressionAfterSpin(data) {
  let baseXP = 5 + Math.floor(data.bet / 10);
  ...
  if (PROGRESSION.level >= MAX_LEVEL) return;
  let missions = getMissionsForLevel(PROGRESSION.level);
...
function claimLevelMission(idx) {
  ...
  let reward = missions[idx].reward;
...
function updateProgressionUI() {
  let rank = getCurrentRank();
  let needed = PROGRESSION.level >= MAX_LEVEL ? 1 : xpForLevel(PROGRESSION.level);
  let xpPct = PROGRESSION.level >= MAX_LEVEL ? 100 : Math.min(100, Math.floor((PROGRESSION.xp / needed) * 100));

  let barRank = document.getElementById('prog-bar-rank');
  let barLevel = document.getElementById('prog-bar-level');
  let barFill = document.getElementById('prog-bar-xp-fill');
  let barText = document.getElementById('prog-bar-xp-text');
...
function renderProgressionModal() {
  let rank = getCurrentRank();
  let nextRank = getNextRank();
  let needed = PROGRESSION.level >= MAX_LEVEL ? 1 : xpForLevel(PROGRESSION.level);
  let xpPct = PROGRESSION.level >= MAX_LEVEL ? 100 : Math.min(100, Math.floor((PROGRESSION.xp / needed) * 100));

  let rankIcon = document.getElementById('prog-rank-icon');
  let rankName = document.getElementById('prog-rank-name');
  let levelEl = document.getElementById('prog-level');
  let rankNext = document.getElementById('prog-rank-next');
  let xpCurrent = document.getElementById('prog-xp-current');
  let xpNeeded = document.getElementById('prog-xp-needed');
  let xpFill = document.getElementById('prog-xp-fill');
  let missionsLevel = document.getElementById('prog-missions-level');
  let rewardPreview = document.getElementById('prog-reward-preview');
  let diffInfo = document.getElementById('prog-difficulty-info');
  ...
  let missions = getMissionsForLevel(PROGRESSION.level);
  let container = document.getElementById('prog-missions-list');
  if (container) {
    container.innerHTML = '';
    if (PROGRESSION.level >= MAX_LEVEL) {
      ...
      missions.forEach(function (mission, idx) {
        let prog = PROGRESSION.missionProgress[idx] || 0;
        let isClaimed = PROGRESSION.missionClaimed[idx];
        let displayProg = mission.key === 'chipTarget' ? Math.min(prog, mission.target) : prog;
        let isComplete = prog >= mission.target;
        let pct = Math.min(100, Math.round((prog / mission.target) * 100));
  ...
  if (rewardPreview) {
    if (PROGRESSION.level >= MAX_LEVEL) {
      ...
      let nextLevel = PROGRESSION.level + 1;
      let nextReward = getLevelReward(nextLevel);
      let isCP = isCheckpointLevel(nextLevel);
      let html = `<div class="prog-reward-item${isCP ? ' prog-reward-checkpoint' : ''}">`;
      ...
      let nextCP = [15, 30, 60].filter(function (cp) { return cp > PROGRESSION.level; })[0];
      if (nextCP && nextCP !== nextLevel) {
        let cpReward = getLevelReward(nextCP);
  ...
  if (diffInfo) {
    let factor = getDifficultyFactor();
    let diffPct = Math.round((1 - factor) * 100);
    let diffLabel = diffPct === 0 ? 'Normal' : 'Rare symbols ' + diffPct + '% harder to hit';
...
let THEME_STORAGE_KEY = 'safariThemeV1';
let VALID_THEMES = ['default', 'jungle', 'neon'];
...
let themeCards = document.querySelectorAll('.theme-card');
for (let tc = 0; tc < themeCards.length; tc++) {
  (function (card) {
    card.addEventListener('click', function () {
      let theme = card.getAttribute('data-theme');
...
function getOrCreatePlayerId() {
  let id = localStorage.getItem('slotPlayerId');
  if (!id) {
...
function setPlayerName(name) {
  let trimmed = (name || '').trim().slice(0, 24);
  if (trimmed) {
...
let MY_PLAYER_ID = getOrCreatePlayerId();

// ── SIMULATED FRIENDS ────────────────────────────────────────────────────────

let SIMULATED_FRIENDS = [
...
function getSortedLBData() {
  let me = getLBPlayerData();
  let pool = LB_TAB === 'friends'
...
function renderLeaderboard() {
  ...
  if (!list) return;
  let data = getSortedLBData();
  let html = '';
  for (let i = 0; i < data.length; i++) {
    let p = data[i];
    let rank = i + 1;
    let rowClass = p.isMe ? 'lb-row lb-row-me' : 'lb-row';
    let medal = rank === 1 ? '🥇' : rank === 2 ? '🥈' : rank === 3 ? '🥉' : `#${rank}`;
    let statVal = LB_SORT === 'totalWon'      ? `${(p.totalWon || 0).toLocaleString()} chips`
                : LB_SORT === 'level'         ? `Level ${p.level || 1}`
                : LB_SORT === 'bestWinStreak' ? `${p.bestWinStreak || 0} streak`
                :                               `${(p.spins || 0).toLocaleString()} spins`;
    let sendBtn = (!p.isMe && LB_TAB === 'friends')
  ...
  let sendBtns = list.querySelectorAll('.lb-send-btn');
  for (let s = 0; s < sendBtns.length; s++) {
    (function (btn) {
      btn.addEventListener('click', function () {
...
function updateLBSortUI() {
  let sorts = document.querySelectorAll('.lb-sort');
...
let lbTabEls = document.querySelectorAll('.lb-tab');
for (let lbti = 0; lbti < lbTabEls.length; lbti++) {
  (function (tab) {
...
let lbSortEls = document.querySelectorAll('.lb-sort');
for (let lbsi = 0; lbsi < lbSortEls.length; lbsi++) {
  (function (sortEl) {
...
let SEND_CHIPS_FRIEND_ID = null;

function openSendChipsModal(friendId) {
  SEND_CHIPS_FRIEND_ID = friendId || null;
  let friend = null;
  for (let i = 0; i < SIMULATED_FRIENDS.length; i++) {
    if (SIMULATED_FRIENDS[i].id === friendId) { friend = SIMULATED_FRIENDS[i]; break; }
  }
  let recipEl = document.getElementById('send-chips-to');
  if (recipEl) {
    recipEl.textContent = 'To: ' + (friend ? (friend.avatar + ' ' + friend.name) : '🎯 A Friend');
  }
  let customInput = document.getElementById('send-chips-amount');
  if (customInput) customInput.value = '';
  let errEl = document.getElementById('send-chips-error');
...
function confirmSendChips(amount) {
  ...
  let fname = friend ? friend.name : 'your friend';
...
document.getElementById('confirm-send-chips-btn').addEventListener('click', function () {
  let customInput = document.getElementById('send-chips-amount');
  let amt = customInput ? parseInt(customInput.value, 10) : 0;
  playSound('click');
  confirmSendChips(amt);
});

let presetBtns = document.querySelectorAll('.send-chip-preset');
for (let spi = 0; spi < presetBtns.length; spi++) {
  (function (btn) {
    btn.addEventListener('click', function () {
      playSound('click');
...
let ARENA_WIN_RANK = { jackpot: 5, bigwin: 4, win: 3, pair: 2, lose: 1 };
...
function getArenaOpponents() {
  let pool = SIMULATED_FRIENDS.slice();
  let buf = new Uint8Array(pool.length);
  crypto.getRandomValues(buf);
...
function resetArenaToLobby() {
  ...
  let codeEl = document.getElementById('arena-room-code');
...
function renderArenaParticipants() {
  let container = document.getElementById('arena-participants');
  if (!container) return;
  let me = {
    id: MY_PLAYER_ID,
    name: getPlayerName(),
  ...
  let all = [me].concat(ARENA_STATE.opponents);
  let html = '';
  for (let i = 0; i < all.length; i++) {
    let p = all[i];
    let cls = p.isMe ? 'arena-player-card arena-player-card-me' : 'arena-player-card';
    html += `<div class="${cls}">`
...
function updateArenaPool() {
  let totalParticipants = 1 + ARENA_STATE.opponents.length;
  let pool = ARENA_STATE.stake * totalParticipants;
  let poolEl = document.getElementById('arena-pool-display');
  if (poolEl) poolEl.textContent = "🪙 Pool: " + pool.toLocaleString() + " chips";
}

function updateArenaStakeUI() {
  let btns = document.querySelectorAll('.arena-stake-btn');
  for (let i = 0; i < btns.length; i++) {
    let val = parseInt(btns[i].getAttribute('data-stake'), 10);
...
function simulateOpponentSpin() {
  ...
  let pay = calcPayout(reels);
  let winType = pay.type || 'lose';
...
function renderArenaBattle() {
  let list = document.getElementById('arena-battle-list');
  if (!list) return;
  let me = { id: MY_PLAYER_ID, name: getPlayerName(), avatar: '🎯', isMe: true };
  let all = [me].concat(ARENA_STATE.opponents);
  let html = '';
  for (let i = 0; i < all.length; i++) {
    let p = all[i];
    let cls = p.isMe ? 'arena-battle-card arena-battle-card-me' : 'arena-battle-card';
    html += `<div class="${cls}">`
...
function resolveArenaBattle() {
  let myReels = [];
  for (let r = 0; r < 3; r++) myReels.push(weightedSymbol());
  let myPay = calcPayout(myReels);
  let myType = myPay.type || 'lose';
  let myPayout = myPay.mult * ARENA_STATE.stake;
  ...
  for (let i = 0; i < ARENA_STATE.opponents.length; i++) {
    let opp = ARENA_STATE.opponents[i];
    let spin = simulateOpponentSpin();
    results.push({
      id: opp.id,
      name: opp.name,
      avatar: opp.avatar,
  ...
  results.sort(function (a, b) {
    let ra = ARENA_WIN_RANK[a.type] || 0;
    let rb = ARENA_WIN_RANK[b.type] || 0;
  ...
  let totalParticipants = 1 + ARENA_STATE.opponents.length;
  let pool = ARENA_STATE.stake * totalParticipants;
  // Prize capped to 2× stake (player's entry + one house match) so virtual
  // opponent contributions don't create guaranteed positive EV.
  let prize = ARENA_STATE.stake * 2;
  let winner = results[0];
  let iWon = winner.isMe;
  ...
  renderArenaResults(pool, prize, iWon);
}

function renderArenaResults(pool, prize, iWon) {
  let msgEl = document.getElementById('arena-result-msg');
  if (msgEl) {
    msgEl.textContent = iWon ? '🏆 You won the arena!' : '💀 Better luck next time!';
    msgEl.className = 'arena-result-msg ' + (iWon ? 'arena-result-win' : 'arena-result-lose');
  }

  let poolEl = document.getElementById('arena-result-pool');
  if (poolEl) poolEl.textContent = iWon
    ? `🏆 Your Prize: ${prize.toLocaleString()} chips`
    : `🪙 Pool: ${pool.toLocaleString()} chips`;

  let list = document.getElementById('arena-result-list');
  if (!list) return;
  let html = '';
  for (let i = 0; i < ARENA_STATE.results.length; i++) {
    let p = ARENA_STATE.results[i];
    let rowClass = 'arena-result-row' + (i === 0 ? ' arena-result-row-winner' : '') + (p.isMe ? ' arena-result-row-me' : '');
    let symStr = p.syms ? p.syms.map(function (idx) { return SYMBOLS[idx] ? SYMBOLS[idx].sym : '?'; }).join(' ') : '? ? ?';
    let outcomeClass = 'arena-result-outcome ' + (p.type || 'lose');
    let outcomeLabel = p.type === 'jackpot' ? '💰 JACKPOT'
...
let stakeBtns = document.querySelectorAll('.arena-stake-btn');
for (let stki = 0; stki < stakeBtns.length; stki++) {
  (function (btn) {
    btn.addEventListener('click', function () {
...
function buildInviteLink(roomCode) {
  ...
  let ref = MY_PLAYER_ID.slice(-6).toUpperCase();
  let url = base + '?ref=' + ref;
